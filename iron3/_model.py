...
class Iron3Network(ContinuousNetwork):
    def __init__(self, backbone: Module, **kwargs: Any) -> None:
        super(Iron3Network, self).__init__(**kwargs)
    ...
    def input_roi(self) -> Tuple[Tuple[int, int], ...]:
        return (tuple(map(int, self.model[0].input_roi)),)  # type: ignore
...
class Iron3NetworkMobileNetV3Style(Iron3Network):
    def __init__(self, **kwargs: Any) -> None:
        super(Iron3NetworkMobileNetV3Style, self).__init__(backbone=MobileNetV3Small(in_channels=1, num_classes=1), **kwargs)


class Iron3NetworkSqueezeNetStyle(Iron3Network):
    def __init__(self, **kwargs: Any) -> None:
        super(Iron3NetworkSqueezeNetStyle, self).__init__(backbone=SqueezeNet1_1_1d(in_channels=1, num_classes=1), **kwargs)


class Iron3NetworkVgg11Style(Iron3Network):
    def __init__(self, **kwargs: Any) -> None:
        super(Iron3NetworkVgg11Style, self).__init__(backbone=VGG11(in_channels=1, num_classes=1), **kwargs)
...
class Iron3EstimationFunction(EstimationFunction):
    def __init__(self, **kwargs: Any) -> None:
        super(Iron3EstimationFunction, self).__init__(Iron3Network, **kwargs)
    ...
    def _check_and_reshape_calibrated_pmf(self, **kwargs: CalibratedDistributions) -> Tuple[torch.Tensor, ...]:
        ...
        for name, calibrated_pmf in kwargs.items():
            if calibrated_pmf is None or not isinstance(calibrated_pmf, torch.Tensor) or calibrated_pmf.dtype != torch.float32 or calibrated_pmf.dim() != 2 or calibrated_pmf.shape[-1:] != (256,):
                raise ValueError(f"The {name} argument is not a valid batch of calibrated distribution with shape (batch_size, 256)")
    ...
    def _check_and_reshape_pmfs(self, **kwargs: Distribution) -> Tuple[torch.Tensor, ...]:
        ...
        for name, pmf in kwargs.items():
            if pmf is None or not isinstance(pmf, torch.Tensor) or pmf.dtype != torch.float32 or pmf.dim() != 1 or pmf.shape[-1:] != (256,):
                raise ValueError(f"The {name} argument is not a valid distribution with shape (256,)")
            size, = pmf.shape
